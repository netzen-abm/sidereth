...
mod tests {
    ...
    fn exact_expiry_is_rejected() {
        assert_eq!(
            validate_authorization(&request(), &allowed(), 1_100),
            Err(AuthorizationValidationError::Expired)
        );
    }
    ...
    fn deny_is_rejected() {
        ...
        assert_eq!(
            validate_authorization(&request(), &result, 1_050),
            Err(AuthorizationValidationError::Denied)
        );
    }
    ...
    fn malformed_request_fails_closed() {
        ...
        assert_eq!(
            validate_authorization(&request, &allowed(), 1_050),
            Err(AuthorizationValidationError::InvalidRequest)
        );
    }
}
