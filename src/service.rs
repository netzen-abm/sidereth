...
mod tests {
    ...
    fn request() -> AuthorizationRequest {
        AuthorizationRequest {
            ...
            subject_ref: reference(ResourceType::Party, "user-1"),
    ...
    fn allowed_result(request: &AuthorizationRequest) -> AuthorizationResult {
        AuthorizationResult {
            ...
            expires_at_epoch_seconds: Some(170),
        }
    }

    #[test]
    fn canonical_allow_is_accepted_only_for_exact_context() {
        let request = request();
        assert_eq!(authorize(&allowed_result(&request), &request, 110), Ok(()));
    }

    #[test]
    fn subject_mismatch_cannot_reuse_allow_result() {
        let request = request();
        let mut changed = request.clone();
        changed.subject_ref = reference(ResourceType::Party, "user-2");
        assert_eq!(
            authorize(&allowed_result(&request), &changed, 110),
            Err(ServiceError::AuthorizationMismatch)
        );
    }

    #[test]
    fn purpose_mismatch_cannot_reuse_allow_result() {
        let request = request();
        let mut changed = request.clone();
    ...
    #[test]
    fn resource_mismatch_cannot_reuse_allow_result() {
        let request = request();
        let mut changed = request.clone();
        changed.resource_ref = reference(ResourceType::Case, "case-2");
        assert_eq!(
            authorize(&allowed_result(&request), &changed, 110),
            Err(ServiceError::AuthorizationMismatch)
        );
    ...
    #[test]
    fn expired_allow_is_rejected() {
        let request = request();
        assert_eq!(
            authorize(&allowed_result(&request), &request, 171),
            Err(ServiceError::AuthorizationExpired)
        );
    }

    #[test]
    fn deny_and_not_applicable_are_not_authorization() {
        let request = request();
        for decision in [AuthorizationDecision::Deny, AuthorizationDecision::NotApplicable] {
            let mut result = allowed_result(&request);
            result.decision = decision;
            assert_eq!(
                authorize(&result, &request, 110),
                Err(ServiceError::AuthorizationDenied)
            );
        }
    }
