...
pub enum ResourceType {
    Case, Incident, Event, Evidence, Authority, Jurisdiction, Party, PartyRelationship,
    Document, Action, Deadline, Response, Escalation, Remedy, Resolution, Procedure,
    ComplianceRequirement, LegalSource, Timeline, Audit, Provenance, Idempotency, Other,
}
...
pub use persistence::{CaseStore, EventStore, IdempotencyClaim, IdempotencyStore, IncidentStore, Persisted, PersistenceError, ResourceLink, ResourceWrite, ResourceWriteMode, Revision, Transaction, TransactionFactory, UnitOfWork, UnitOfWorkContext, UnitOfWorkError, UnitOfWorkFactory};
...
pub use tool_registry::{InMemoryToolRegistry, ToolDataClass, ToolDependency, ToolImplementation, ToolLifecycle, ToolRegistryAuditRecord, ToolRegistryCriteria, ToolRegistryEntry, ToolRegistryError, ToolRiskClass, ToolVersion, ToolVersionRequirement, ToolExecutionMode};
...
mod tests {
    use super::*;
    #[test] fn resource_ref_is_explicit_and_stable_on_wire(){let r=ResourceRef::new(ResourceType::Document,"doc-1").unwrap();let j=serde_json::to_string(&r).unwrap();assert_eq!(j,r#"{"resource_type":"document","id":"doc-1"}"#);assert_eq!(serde_json::from_str::<ResourceRef>(&j).unwrap(),r);}
    #[test] fn empty_resource_ref_is_rejected(){assert_eq!(ResourceRef::new(ResourceType::Case,""),Err("resource reference id is required"));}
    #[test] fn party_relationship_resource_type_has_canonical_wire_value(){assert_eq!(serde_json::to_string(&ResourceType::PartyRelationship).unwrap(),"\"party_relationship\"");}
}
