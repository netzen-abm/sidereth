...
pub struct ResourceRef { pub resource_type: ResourceType, pub id: Id }
impl ResourceRef {
...
pub enum ResourceType {
    Case, Incident, Event, Evidence, Authority, Jurisdiction, Party, Document, Action,
    Deadline, Response, Escalation, Remedy, Resolution, Procedure, ComplianceRequirement,
    LegalSource, Timeline, Other,
}
...
pub use persistence::{CaseStore, EventStore, IdempotencyClaim, IdempotencyStore, IncidentStore, Persisted, PersistenceError, Revision, Transaction, TransactionFactory};
...
impl Case {
    pub fn new(case_id: Id) -> Result<Self, &'static str> { if case_id.is_empty(){return Err("case id is required");} Ok(Self{case_id,state:CaseState::Draft}) }
    pub fn transition(&mut self,next:CaseState)->Result<(), &'static str>{if !self.state.can_transition_to(&next){return Err("invalid case state transition");}self.state=next;Ok(())}
}
...
#[cfg(test)]
mod tests { use super::*; #[test] fn resource_ref_is_explicit_and_stable_on_wire(){let reference=ResourceRef::new(ResourceType::Document,"doc-1").unwrap();let json=serde_json::to_string(&reference).unwrap();assert_eq!(json,r#"{"resource_type":"document","id":"doc-1"}"#);let decoded:ResourceRef=serde_json::from_str(&json).unwrap();assert_eq!(decoded,reference);} #[test] fn empty_resource_ref_is_rejected(){assert_eq!(ResourceRef::new(ResourceType::Case,""),Err("resource reference id is required"));} }
