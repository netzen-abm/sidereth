...
#[derive(Debug)]
pub struct PostgresUnitOfWorkFactory { connection_string: String }
...
impl PostgresUnitOfWorkContext {
    fn resource_type_name(resource_type: ResourceType) -> &'static str {
        match resource_type {
            ResourceType::Case => "case", ResourceType::Incident => "incident", ResourceType::Event => "event",
            ResourceType::Authority => "authority", ResourceType::Jurisdiction => "jurisdiction", ResourceType::Party => "party",
            ResourceType::Document => "document", ResourceType::Action => "action", ResourceType::Deadline => "deadline",
            ResourceType::Response => "response", ResourceType::Escalation => "escalation", ResourceType::Remedy => "remedy",
            ResourceType::Resolution => "resolution", ResourceType::Procedure => "procedure",
            ResourceType::ComplianceRequirement => "compliance_requirement", ResourceType::LegalSource => "legal_source",
            ResourceType::Timeline => "timeline", ResourceType::Evidence => "evidence", ResourceType::Audit => "audit",
            ResourceType::Provenance => "provenance", ResourceType::Idempotency => "idempotency", ResourceType::Other => "other",
        }
    }
    fn resource_ref(resource_type: &str, id: String) -> Result<ResourceRef, UnitOfWorkError> {
        let kind = match resource_type {
            ...
            "document" => ResourceType::Document, "action" => ResourceType::Action, "deadline" => ResourceType::Deadline,
            "response" => ResourceType::Response, "escalation" => ResourceType::Escalation, "remedy" => ResourceType::Remedy,
            "resolution" => ResourceType::Resolution, "procedure" => ResourceType::Procedure,
...
impl UnitOfWorkContext for PostgresUnitOfWorkContext {
    fn read_resource(&mut self, resource_ref: &ResourceRef) -> Result<Option<ResourceRecord>, UnitOfWorkError> {
        ...
        let row = client.query_opt(
            "SELECT resource_type, resource_id, schema_version, revision, payload FROM sidereth_resource_records WHERE resource_type = $1 AND resource_id = $2",
    ...
    fn write_resource(&mut self, write: ResourceWrite) -> Result<(), UnitOfWorkError> {
        ...
        match (write.mode, write.expected_revision) {
            ...
            (ResourceWriteMode::Insert, None) => client.execute(
                "INSERT INTO sidereth_resource_records (resource_type, resource_id, schema_version, revision, payload) VALUES ($1, $2, $3, 0, $4)",
                &[&resource_type, &id, &schema_version, &payload],
            ).map(|_| ()).map_err(|error| UnitOfWorkError::Persistence(PostgresUnitOfWork::map_error(error))),
            (ResourceWriteMode::Upsert, Some(expected)) => {
                ...
                let affected = client.execute(
                    "UPDATE sidereth_resource_records SET schema_version = $3, revision = $4, payload = $5, updated_at = CURRENT_TIMESTAMP WHERE resource_type = $1 AND resource_id = $2 AND revision = $6",
                    &[&resource_type, &id, &schema_version, &next_revision, &payload, &expected_revision],
                ).map_err(|error| UnitOfWorkError::Persistence(PostgresUnitOfWork::map_error(error)))?;
                if affected == 0 { return Err(UnitOfWorkError::Persistence(PersistenceError::Conflict)); }
                Ok(())
            }
            (ResourceWriteMode::Upsert, None) => client.execute(
                "INSERT INTO sidereth_resource_records (resource_type, resource_id, schema_version, revision, payload) VALUES ($1, $2, $3, 0, $4) ON CONFLICT (resource_type, resource_id) DO UPDATE SET schema_version = EXCLUDED.schema_version, revision = sidereth_resource_records.revision + 1, payload = EXCLUDED.payload, updated_at = CURRENT_TIMESTAMP",
                &[&resource_type, &id, &schema_version, &payload],
            ).map(|_| ()).map_err(|error| UnitOfWorkError::Persistence(PostgresUnitOfWork::map_error(error))),
        }
    }
    ...
    fn link_resources(&mut self, link: ResourceLink) -> Result<(), UnitOfWorkError> {
        ...
        let mut client = self.client.lock().map_err(|_| UnitOfWorkError::Persistence(PersistenceError::Unavailable))?;
        client.execute(
            "INSERT INTO sidereth_resource_links (source_type, source_id, relation, target_type, target_id) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (source_type, source_id, relation, target_type, target_id) DO NOTHING",
            &[&source_type, &link.source_ref.id, &link.relation, &target_type, &link.target_ref.id],
        ).map(|_| ()).map_err(|error| UnitOfWorkError::Persistence(PostgresUnitOfWork::map_error(error)))
    }
}
...
impl UnitOfWork for PostgresUnitOfWork {
    ...
    where F: FnOnce(&mut Self::Context) -> Result<R, UnitOfWorkError> {
        if !self.active { return Err(UnitOfWorkError::Persistence(PersistenceError::Conflict)); }
        let mut context = PostgresUnitOfWorkContext { client: Arc::clone(&self.client) };
        match operation(&mut context) {
            Ok(value) => Ok(value),
            Err(error) => { let _ = self.rollback_in_place(); Err(error) }
        }
    }
    fn commit(mut self) -> Result<(), PersistenceError> {
        if !self.active { return Err(PersistenceError::Conflict); }
        let result = { let mut client = self.client.lock().map_err(|_| PersistenceError::Unavailable)?; client.batch_execute("COMMIT").map_err(Self::map_error) };
        result?; self.active = false; Ok(())
    }
...
impl PostgresUnitOfWork {
    fn rollback_in_place(&mut self) -> Result<(), PersistenceError> {
        ...
        let result = { let mut client = self.lock_client().map_err(|_| PersistenceError::Unavailable)?; let result = client.batch_execute("ROLLBACK").map_err(Self::map_error); drop(client); result };
        result?; self.active = false; Ok(())
    }
}
...
impl UnitOfWorkFactory for PostgresUnitOfWorkFactory {
    ...
    fn begin(&mut self) -> Result<Self::Uow, PersistenceError> {
        let mut client = Client::connect(&self.connection_string, NoTls).map_err(PostgresUnitOfWork::map_error)?;
        client.batch_execute("BEGIN").map_err(PostgresUnitOfWork::map_error)?;
        Ok(PostgresUnitOfWork { client: Arc::new(Mutex::new(client)), active: true })
    }
}
