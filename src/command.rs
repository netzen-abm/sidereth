...
mod tests {
    ...
    struct MockUow { context: MockContext, fail_rollback: bool, committed: bool, rolled_back: bool }
    ...
    impl UnitOfWork for MockUow {
        ...
        fn commit(mut self) -> Result<(), PersistenceError> { self.committed = true; Ok(()) }
        fn rollback(mut self) -> Result<(), PersistenceError> { self.rolled_back = true; if self.fail_rollback { Err(PersistenceError::Unavailable) } else { Ok(()) } }
    }

    #[derive(Default)]
    struct MockFactory { fail_rollback: bool }

    impl UnitOfWorkFactory for MockFactory {
        type Uow = MockUow;
        fn begin(&mut self) -> Result<Self::Uow, PersistenceError> { Ok(MockUow { context: MockContext::default(), fail_rollback: self.fail_rollback, committed: false, rolled_back: false }) }
    }
    ...
    fn plan_contains_idempotency_and_domain_side_effects() {
        ...
        plan.insert_resource(ResourceRef::new(ResourceType::Event, "event-1").unwrap(), 1, serde_json::json!({ "event_type": "case.created" })).unwrap();
        plan.insert_resource(ResourceRef::new(ResourceType::Audit, "audit-1").unwrap(), 1, serde_json::json!({ "action": "case.created" })).unwrap();
        plan.insert_resource(ResourceRef::new(ResourceType::Provenance, "prov-1").unwrap(), 1, serde_json::json!({ "operation": "case.create" })).unwrap();
    ...
    fn executor_rolls_back_when_build_fails() {
        ...
        let result = execute_authoritative_command(&mut factory, plan, |_context, _| Err(UnitOfWorkError::InvalidOperation));
